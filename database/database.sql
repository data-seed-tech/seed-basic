...
) ENGINE=InnoDB DEFAULT CHARSET=UTF8MB4 COLLATE=utf8mb4_unicode_ci COMMENT='SEED Apps definitions';
...
CREATE TABLE `seed_menus` (
 `menuId` int NOT NULL AUTO_INCREMENT COMMENT 'PK',
...
) ENGINE=InnoDB DEFAULT CHARSET=UTF8MB4 COLLATE=utf8mb4_unicode_ci COMMENT='Seed App menus configuration. A menu is a direct link for editien records for the specified tableName. The menu text is the one specified in menuText.';
...
CREATE TABLE `seed_nomenclatures` (
  `nomenclatureId` int NOT NULL AUTO_INCREMENT COMMENT 'Primary Key',
  `tableName` varchar(50) NOT NULL COMMENT 'The table that can be references this nomenclature',
  `filterText` varchar(50) NOT NULL COMMENT 'Text displayed for the filter menu created automatically',
